...
def encode_class_values(cs: list[str], class_index: dict[str, int]) -> np.array:
    ...
    k = len(class_index)
    n = len(cs)
    encoded_values = np.zeros((n, k), dtype=int)

    for i in range(n):
        encoded_values[i, class_index[cs[i]]] = 1
...
def predict_probabilities(W_h: np.array, W_o: np.array, xs: np.array) -> np.array:
    ...
    a_h_bias = np.column_stack([np.ones(len(a_h)), a_h])
    
    z_o = np.dot(a_h_bias, W_o.T)
    a_o = sigmoid(z_o)
...
if __name__ == "__main__":
    ...
    print("(e)")
    best_model_index = np.argmin(validation_misclassification_rates) # TODO (e): replace -1 (last model) with your code
